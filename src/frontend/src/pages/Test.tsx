            ...
            <div className="flex items-center justify-center gap-3 px-5 pb-4 pt-3">
              {OPTION_LABELS.map((label, optIdx) => {
                ...
                return (
                  <div
                    key={optIdx}
                    className={`w-11 h-11 rounded-lg border-2 font-bold text-sm flex items-center justify-center shrink-0 ${buttonClass}`}
                  >
                    {label}
                  </div>
                );
...
export default function Test() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="sticky top-16 z-30 bg-white border-b border-gray-100 shadow-sm shrink-0">
        <div className="max-w-3xl mx-auto px-4 py-2 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <span className="bg-navy text-white text-xs font-bold px-2.5 py-1 rounded-full shrink-0">
        ...
        <div className="max-w-3xl mx-auto px-4 py-2 flex gap-1.5 overflow-x-auto scrollbar-hide">
          {questions.map((_, i) => (
            <button
              ...
              className={`w-7 h-7 rounded-full text-xs font-bold transition-all shrink-0 ${
                i === currentIndex
      ...
      {/* Main content: Full-screen question image with fixed bottom buttons */}
      <div className="flex-1 w-full flex flex-col overflow-hidden relative">
        {/* Question image container — uses h-[90vh] */}
        <div className="w-full bg-white overflow-y-auto flex items-start justify-center pb-28 px-2">
          {currentQuestion.questionImageData ? (
            <img
              src={currentQuestion.questionImageData}
              alt={`Question ${currentIndex + 1} of ${totalQuestions}`}
              className="w-full h-auto object-contain block border border-navy/10 rounded-2xl mt-2"
        ...
        {/* Fixed bottom bar: A/B/C/D circular answer buttons + navigation */}
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 flex items-center justify-center gap-3 bg-white/95 backdrop-blur-sm px-4 py-3 rounded-full shadow-lg border border-navy/10">
          {/* Previous button */}
          <Button
            variant="outline"
            ...
            size="sm"
            className="rounded-full w-9 h-9 p-0 border-navy/30 text-navy hover:bg-navy/5 disabled:opacity-40 shrink-0"
          >
            <ChevronLeft className="h-4 w-4" />
          </Button>

          {/* Small circular A/B/C/D answer buttons */}
          {['A', 'B', 'C', 'D'].map((label, idx) => {
            ...
            return (
              <button
                ...
                className={`w-12 h-12 rounded-full border-2 font-bold text-sm transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-navy/40 flex items-center justify-center shrink-0 ${
                  isSelected
                    ? 'border-navy bg-navy text-white shadow-lg scale-110'
                    : 'border-gray-300 bg-white text-gray-600 hover:border-navy/50 hover:bg-navy/5 active:scale-95'
                }`}
          ...
          {currentIndex < totalQuestions - 1 ? (
            <Button
              ...
              size="sm"
              className="rounded-full w-9 h-9 p-0 bg-navy hover:bg-navy-dark text-white shrink-0"
            >
            ...
            <Button
              ...
              size="sm"
              className="rounded-full w-9 h-9 p-0 bg-green-600 hover:bg-green-700 text-white shrink-0"
            >
